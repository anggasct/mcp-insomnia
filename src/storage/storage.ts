...
import { CollectionStructure, InsomniaExecution, InsomniaRequest, InsomniaEnvironment } from '../types/index.js';

const MAX_HISTORY_LENGTH = 20;

interface StoredCollectionsData {
    [key: string]: CollectionStructure;
}
...
interface ExportData {
    ...
    collections: StoredCollectionsData;
}
...
export class PersistentStorage {
    ...
    private readCollectionsFromFile(): Map<string, CollectionStructure> {
        try {
            ...
            const collectionsData = JSON.parse(data) as StoredCollectionsData;
    ...
    private writeCollectionsToFile(collections: Map<string, CollectionStructure>): void {
        try {
            ...
            fs.writeFileSync(this.collectionsFile, JSON.stringify(collectionsData, null, 2), 'utf-8');
        } catch (error) {
    ...
    public addExecution(collectionId: string, requestId: string, execution: InsomniaExecution): boolean {
        ...
        const request = collection.requests.find((r: InsomniaRequest) => r._id === requestId);
        if (!request) {
            return false;
        }
